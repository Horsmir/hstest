	...
	QObject(parent)
{
	...
	logOut = new LogFileOut("../../var/log/hstest-server.log", this);
...
QString TestManager::getStudentDbFileName() const
{
	return studentDbFileName;
}
...
bool TestManager::readStudentDbFromFile()
{
	...
	if(!file.exists())
	{
		logOut->write("ERROR: File " + testsDir + "/" + studentDbFileName + " no exist.");
		return false;
	}
	if(!file.open(QIODevice::ReadOnly))
	{
...
void TestManager::setStudentDbFileName(const QString &studentDbFileName)
{
	this->studentDbFileName = studentDbFileName;
}
